 ...
 * <?php
 * // 检查验证码 
 * 	if (!sessCode::check(@$_POST['secode'])) { 
 * 	    print 'error secode'; 
 * 	}
 * ?>
 */ 
...
class sessCode {  
     ...
     * @var string 
     */  
    public static $seKey = 'defaultSessKey';  
    ...
    public static $imageH = 50;       // 验证码图片宽  
    public static $imageL = 200;      // 验证码图片长  
    public static $length = 4;        // 验证码位数  
    public static $ttf='ariblk.ttf';  // 验证码字体
    ...
    public static function entry() {
        ...
        // 验证码使用随机字体   
        $ttf = DOCUROOT . '/include/fonts/' . self::$ttf ;   
        if(!is_file($ttf)) exit("Can't find <b>{$ttf}</b>!");
    	...
    	if(isset($_GET['debug'])&&isset($_SESSION['UserLevel'])){
			if( $_GET['debug']=='showmsg' && $_SESSION['UserLevel']==6 ) exit;
		}
    ...
    public static function check($code) {  
        func_initSession();
        // 验证码不能为空  
        if(empty($code) || empty($_SESSION[self::$seKey]['code'])) { 
            return false;  
        }  
        // session 过期  
        ...
        if(strtolower($code) == strtolower($_SESSION[self::$seKey]['code'])) {  
            //重复验证无效
        	unset($_SESSION[self::$seKey]['code']);
            return true;  
        }  
