...
class files {
	 ...
	 * 获取远程HTTP文件大小
	 * */
	public function getHttpFileSize($url){
		$url = parse_url($url);
	 ...
	 * 获取远程FTP文件大小
	 * */
	public function getFtpFileSize($host,$file,$user='anonymous',$pass='anonymous'){
	...
	### 删除空目录
	function delEmptyFolder( $dir ){exit;//有重大问题
	if( $handle = @opendir( $dir ) ){
		while( ( $file = @readdir( $handle ) ) !== false ){
			if( $file != "." && $file != ".." ){
				$filepath = $dir."/".$file;
